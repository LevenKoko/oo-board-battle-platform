<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; line-height: 1.6; max-width: 1000px; margin: 0 auto; padding: 40px; background-color: #ffffff; box-shadow: 0 0 20px rgba(0,0,0,0.05);">

<div style="text-align: center; padding-bottom: 40px; border-bottom: 2px solid #f0f0f0; margin-bottom: 40px;">
    <h1 style="font-size: 48px; margin-bottom: 10px; color: #2c3e50; letter-spacing: -1px;">Zenith Board Battle Platform</h1>
    <h2 style="font-size: 24px; font-weight: 300; color: #7f8c8d; margin-top: 0;">Phase II Detailed Design & Implementation Report</h2>
...
### 1.1 项目背景与愿景 (Background & Vision)
本项目 **Zenith Board Battle Platform** 旨在通过**面向对象 (Object-Oriented)** 的设计思想，解决多棋种规则的统一抽象、即时状态同步的复杂性以及 AI 策略的灵活扩展问题。

我们的愿景是打造一个不仅能玩游戏，更能作为**现代软件工程实践范本**的系统。它集成了用户认证、即时通讯、智能决策和数据持久化等核心模块，展示了如何在一个分布式系统中维持代码的清晰度与健壮性。
...
| **在线大厅** | 房间管理 | ⭐⭐⭐ | 支持创建房间、加入房间、实时列表刷新、空房间自动销毁。 |
| **实时对战** | 状态同步 | ⭐⭐⭐⭐⭐ | 基于 WebSocket 的毫秒级落子同步，支持断线重连与状态恢复。 |
| **智能 AI** | 多级策略 | ⭐⭐⭐⭐ | 集成 Greedy、Minimax (Alpha-Beta 剪枝)、MCTS 算法，支持人机/机机对战。 |
| **交互系统** | 准备/悔棋/投降 | ⭐⭐⭐⭐ | 包含“准备大厅”、“请求交换阵营”、“请求悔棋”等复杂的握手流程。 |
| **回放系统** | 云端存档 | ⭐⭐⭐ | 支持将对局保存至云端，随时加载复盘或继续本地演练。 |

### 1.3 技术选型解析 (Technology Stack Rationale)

*   **Backend (FastAPI)**: 选择 FastAPI 是因为它原生支持异步 (AsyncIO)，这对于处理大量 WebSocket 长连接至关重要。其基于 Pydantic 的类型系统保证了前后端数据契约的严谨性。
*   **Frontend (React + TypeScript)**: TypeScript 的静态类型检查极大减少了前端逻辑错误，React 的组件化架构使得复用 UI（如棋盘、控制栏）变得极其简单。
*   **Communication (WebSocket)**: 传统的 HTTP 轮询无法满足棋类对战的实时性要求。WebSocket 提供了全双工通信通道，实现了服务器主动推送（Server Push）。
*   **Database (MySQL)**: 使用 MySQL 存储用户关系数据，同时利用其 JSON 字段特性存储非结构化的棋局历史 (`moves_json`)，兼顾了关系查询与文档存储的优势。

---

## 2. 系统架构设计 (System Architecture)

本系统采用经典的分层架构模式，强调关注点分离 (Separation of Concerns)。

### 2.1 架构分层 (Layered Architecture)

1.  **表现层 (Presentation Layer)**: 运行在浏览器中的 React 单页应用 (SPA)。负责 UI 渲染、用户交互捕获以及与后端的通信适配。
2.  **应用服务层 (Application Service Layer)**: FastAPI 提供的 REST 接口与 WebSocket 端点。负责路由分发、请求校验、会话管理。
3.  **领域层 (Domain Layer)**: **系统的核心**。包含 `GameController`、`AbstractGame` 及其子类、`AIStrategy`。这一层封装了所有的业务规则，不依赖于具体的 HTTP 框架。
4.  **基础设施层 (Infrastructure Layer)**: 负责数据持久化 (`SQLAlchemy`) 和底层网络通信 (`ConnectionManager`)。

### 2.2 组件交互图 (Component Diagram)
...
### 2.3 混合通信模型 (Hybrid Communication Model)

我们采用了 HTTP 与 WebSocket 互补的通信策略：

*   **HTTP REST**: 用于**资源导向**的操作。例如用户注册、登录（获取 Token）、创建房间（获取 RoomID）、获取回放列表。这些操作是无状态的，适合 Request-Response 模型。
*   **WebSocket**: 用于**事件导向**的操作。例如落子 (`MOVE`)、悔棋 (`UNDO`)、聊天/状态同步。WebSocket 提供了全双工通信通道，能够维持会话上下文 (`RoomSession`)，是实现在线对战的关键。


---

## 3. 面向对象设计与模式应用 (OO Design & Patterns)

在本平台的架构与实现中，我们深入贯彻了面向对象的设计思想，并通过恰当应用设计模式，有效提升了系统的**可维护性**、**可扩展性**和**鲁棒性**。这部分将结合具体代码结构，详细阐述所选模式及其背后的设计考量。

### 3.1 策略模式 (Strategy Pattern) —— AI 系统的核心
...
*   **Strategy Interface (策略接口)**: `AIStrategy` (一个抽象基类或 Python 协议)。它定义了所有 AI 算法必须遵循的统一接口，即 `make_move(game: AbstractGame, player: Player) -> Tuple[int, int]`。
*   **Concrete Strategies (具体策略)**: 实现了 `AIStrategy` 接口的各个具体 AI 算法类，例如：
    *   `GreedyGomokuAI` / `GreedyReversiAI`: 实现简单的贪心策略。
    *   `MinimaxReversiAI` / `MinimaxGomokuAI`: 实现基于 Alpha-Beta 剪枝的极大极小搜索算法。
    *   `MCTSReversiAI`: (预留/基础实现) 实现蒙特卡洛树搜索算法。

**体现的 OO 原则**:
*   **Open-Closed Principle (OCP - 开闭原则)**: 当需要引入新的 AI 算法（例如，更强大的 MCTS 或基于神经网络的 AI）时，只需创建新的具体策略类，实现 `AIStrategy` 接口，无需修改 `GameController` 或现有的 AI 代码。
*   **Dependency Inversion Principle (DIP - 依赖倒置原则)**: `GameController` 依赖于 `AIStrategy` 抽象，而不是具体的 AI 实现。

### 3.2 模板方法模式 (Template Method) —— 游戏规则的抽象与复用
...
*   **Abstract Class**: `AbstractGame`。它定义了棋类游戏的通用骨架算法 `make_move(x, y)`。这个方法包含一系列步骤，其中一些步骤是通用的，另一些则是抽象的（Hook Methods），需要由子类具体实现。
...
*   **Hook Methods (钩子方法)**:
    ...
    *   `_is_valid_move(x, y)`: 检查具体棋种的落子合法性。
...
### 3.3 单例模式 (Singleton) —— 全局游戏状态的唯一入口
...
*   **作用**: 保证了无论多少个 WebSocket 连接或 HTTP 请求，在处理游戏逻辑时，都操作的是内存中同一份实时游戏状态数据，从而避免了数据不一致和竞态条件。

### 3.4 核心类图 (Class Diagram)
...
### 4.2 实时对战交互协议 (Real-time Protocol)

前后端通过 WebSocket 进行全双工通信，协议采用 JSON 格式。
...
### 4.3 游戏规则引擎 (Game Engine Mechanics)
...
for dx, dy in directions:
    ...
    for i in range(1, size):
        ...
        if grid[ny][nx] == opponent:
            ...
            flippable.extend(line) # 形成夹击，收集
            break
        else:
            break # 遇到空格或边界，未形成夹击
```

#### 4.3.2 悔棋逻辑 (Undo Logic)
...
### 4.4 存档与回放系统 (Save/Load & Replay)

系统支持将当前的棋局状态保存为云端记录或本地文件。

#### 4.4.1 数据序列化 (Serialization)
...
*   **`moves_json`**: 这是一个非结构化字段，存储了完整的棋局历史。
...
| GET | `/api/rooms` | 获取房间列表 | - | `MatchListResponse` |
